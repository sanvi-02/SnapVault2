...
const roleMeta = {
  Admin:        { color: "#f59e0b", bg: "rgba(245,158,11,0.12)", border: "rgba(245,158,11,0.25)", icon: "👑" },
  Photographer: { color: "#8b5cf6", bg: "rgba(139,92,246,0.12)", border: "rgba(139,92,246,0.25)", icon: "📷" },
  ClubMember:   { color: "#22c55e", bg: "rgba(34,197,94,0.12)",  border: "rgba(34,197,94,0.25)",  icon: "🎖️" },
  Viewer:       { color: "#9ca3af", bg: "rgba(156,163,175,0.1)", border: "rgba(156,163,175,0.2)", icon: "👁️" },
};
...
const StatCard = ({ label, value, icon }) => (
  ...
  >
    ...
    <span style={{ fontSize: "24px", fontWeight: "800", color: "var(--text-1)", lineHeight: 1.2 }}>{value}</span>
...
const Profile = () => {
  ...
  const totalLikes = myMedia.reduce((acc, m) => acc + (m.likes?.length || 0), 0);
  const totalDownloads = myMedia.reduce((acc, m) => acc + (m.downloadCount || 0), 0);
    ...
    <div style={{ minHeight: "100vh", background: "var(--bg-1)", color: "var(--text-1)", fontFamily: "Inter,sans-serif" }}>
      ...
      <div style={{ maxWidth: "960px", margin: "0 auto", padding: "0 24px 64px" }}>
        ...
        <div style={{
          ...
          marginTop: "-52px", marginBottom: "32px",
          flexWrap: "wrap",
        }}>
          {/* Avatar */}
          <div style={{
          ...
          <div style={{ flex: 1, paddingBottom: "4px" }}>
            ...
            <p style={{ color: "var(--text-3)", fontSize: "14px", margin: "4px 0 0" }}>
              {user?.email}
            </p>
          </div>
          ...
          {tabs.map(({ key, label, icon }) => (
            <button
              ...
              style={{
                ...
                boxShadow: tab === key ? "0 4px 12px rgba(139,92,246,0.3)" : "none",
                whiteSpace: "nowrap",
              }}
            >
          ...
          <div>
            {mediaLoading ? (
              ...
              <div style={{
                ...
                <div style={{ fontSize: "48px", marginBottom: "14px", animation: "float 3s ease-in-out infinite" }}>🖼️</div>
                <p style={{ color: "var(--text-2)", fontSize: "16px", fontWeight: "600" }}>No uploads yet</p>
              ...
              <>
                <p style={{ color: "var(--text-4)", fontSize: "12px", marginBottom: "16px", fontWeight: "500" }}>
                  {myMedia.length} {myMedia.length === 1 ? "photo" : "photos"} uploaded
                </p>
                <div style={{
                  display: "grid",
                    ...
                    >
                      ...
                      >
                        <div style={{ display: "flex", gap: "8px" }}>
                          <span style={{ fontSize: "11px", color: "#fff", background: "rgba(0,0,0,0.5)", borderRadius: "99px", padding: "2px 8px", fontWeight: "600" }}>
                            ❤️ {item.likes?.length || 0}
                          </span>
                          {item.visibility === "private" && (
                            <span style={{ fontSize: "11px", color: "#f43f5e", background: "rgba(0,0,0,0.5)", borderRadius: "99px", padding: "2px 8px", fontWeight: "600" }}>
                              🔒
                            </span>
                          )}
                        </div>
          ...
          <div>
            ...
            {events.length === 0 ? (
              <div style={{
                ...
                <p style={{ color: "var(--text-2)", fontSize: "16px", fontWeight: "600" }}>No events yet</p>
              </div>
            ) : (
                ...
                {events.slice(0, 9).map((event) => (
                  ...
                  >
                    ...
                    <div style={{ flex: 1, overflow: "hidden" }}>
                      <p style={{ fontSize: "14px", fontWeight: "600", color: "var(--text-1)", margin: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                        {event.name}
                      </p>
                    ...
                    <span style={{ color: "var(--accent-3)", fontSize: "16px" }}>→</span>
                  </Link>
                ))}
                {events.length > 9 && (
                  <Link
                    ...
                    style={{
                      display: "flex", alignItems: "center", justifyContent: "center",
                      textDecoration: "none", color: "var(--accent-3)",
                      background: "var(--bg-2)", borderRadius: "12px",
        ...
        {tab === "manage" && isAdmin && (
          <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
            <p style={{ color: "var(--text-3)", fontSize: "14px", marginBottom: "4px" }}>
            ...
            {[
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  <p style={{ fontSize: "15px", fontWeight: "700", color: "var(--text-1)", margin: 0 }}>{action.title}</p>
                  <p style={{ fontSize: "13px", color: "var(--text-3)", margin: "2px 0 0" }}>{action.desc}</p>
                </div>
                <span style={{ color: action.accent, fontSize: "20px" }}>→</span>
              </Link>
            ))}
          </div>
        )}
